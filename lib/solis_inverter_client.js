...
class SolisInverterClient {
  ...
  /**
   * @return {Promise<{lastSeen: number, remoteServer: {a: boolean, b: boolean}, logger: {mode: string, sta: {rssi: string, ip: string, ssid: string, mac: string}, serial: string, version: string, ap: {ip: string, ssid: string, mac: string}}, inverter: {serial: string, firmwareMain: string, model: string, firmwareSlave: string}, power: number, energy: {total: number, today: number}}>}
   */
  fetchData () {
